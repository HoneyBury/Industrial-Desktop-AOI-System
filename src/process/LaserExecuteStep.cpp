...
StepExecutionResult LaserExecuteStep::execute(WorkflowContext &context) const {
  ...
  for (auto &pointResult : context.laserPointResults) {
    const bool moveOk = context.moveCameraPose
                            ? context.moveCameraPose(pointResult.machinePose, "laser_execute")
                            : (context.motionController->moveAbsolute(MotionAxis::CameraX, pointResult.machinePose.x) &&
