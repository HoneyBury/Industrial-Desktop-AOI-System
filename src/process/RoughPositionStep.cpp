...
StepExecutionResult RoughPositionStep::execute(WorkflowContext &context) const {
  if (context.motionController != nullptr) {
    ...
    const double currentX = context.motionController->position(MotionAxis::CameraX).value_or(0.0);
    const double currentY = context.motionController->position(MotionAxis::CameraY).value_or(0.0);
    ...
    context.currentMachinePose = MechanicalPose {
        context.motionController->position(MotionAxis::CameraX).value_or(0.0),
        context.motionController->position(MotionAxis::CameraY).value_or(0.0),
